...
def route(profile: Profile, action: int, config):
	...
	elif action == ACTIONS.CONFIG_GENERAL:
		account_config_handler.accounts_group_general_settings(profile.accounts, browser, config['timeout'], config['config'])
	elif action == ACTIONS.CONFIG_ACCOUNT:
		account_config_handler.accounts_group_account_settings(profile.accounts, browser, config['timeout'], config['config']['from_alias'])
	...
	elif action == ACTIONS.FILTER_FROM:
		account_filter_handler.accounts_group_filter_from(profile.accounts, browser, config['timeout'], config['filter']['accept_from_name'])
	elif action == ACTIONS.FILTER_BOUNCE:
		account_filter_handler.accounts_group_filter_bounce(profile.accounts, browser, config['timeout'])
